      ...
      if (response.statusCode == 201) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Shop and Initial Owner Created Successfully!')),
          );
          Navigator.pop(context); // Close the form
        }
      } else {
        final err = jsonDecode(response.body)['message'] ?? 'Failed to create shop and owner';
        if (mounted) {
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            const Text('Initial Owner Credentials', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const Text('You must attach a primary Owner to authorize this node.', style: TextStyle(color: Colors.grey)),
            const SizedBox(height: 16),
            TextFormField(
            ...
            ElevatedButton(
              ...
              child: _isLoading
                  ...
                  : const Text('Create Shop & Owner', style: TextStyle(color: Colors.white, fontSize: 18)),
            ),
