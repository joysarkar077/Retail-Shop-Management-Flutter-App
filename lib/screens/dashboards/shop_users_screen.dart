...
class _ShopUsersScreenState extends State<ShopUsersScreen> {
  ...
  Future<void> _fetchShopUsers() async {
    try {
      final response = await ApiService.get(
        '${ApiConfig.baseUrl}/auth/shop/${widget.shopId}/users',
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          '${widget.shopName} Staff | S-${widget.shopId.substring(widget.shopId.length - 6).toUpperCase()}',
        ),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          subtitle: Text(
                            'ID: U-${user['_id'].toString().substring(user['_id'].toString().length - 6).toUpperCase()}\n${user['email']}',
                          ),
                          trailing: Chip(
                            label: Text(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () async {
          ...
          await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) =>
                  RegisterUserScreen(preselectedShopId: widget.shopId),
            ),
          );
