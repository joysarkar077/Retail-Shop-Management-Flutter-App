...
class _ProductListScreenState extends State<ProductListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final canModify = role == 'superadmin' || role == 'admin' || role == 'owner' || role == 'manager';
    ...
    return Scaffold(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text('Stock: ${product['stock_count']} ${product['unit']} | SKU: ${product['sku'] ?? 'N/A'}'),
                            trailing: Text(
                              '৳${product['sellingPrice'].toStringAsFixed(2)}', 
                              style: const TextStyle(color: Colors.green, fontWeight: FontWeight.bold, fontSize: 16)
                            ),
                            onTap: canModify ? () async {
      ...
      floatingActionButton: canModify ? FloatingActionButton.extended(
        backgroundColor: Colors.green[800],
        foregroundColor: Colors.white,
        onPressed: () async {
          final added = await Navigator.push(context, MaterialPageRoute(builder: (_) => const AddEditProductScreen()));
