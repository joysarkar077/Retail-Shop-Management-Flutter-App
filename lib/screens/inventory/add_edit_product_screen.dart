...
class _AddEditProductScreenState extends State<AddEditProductScreen> {
  ...
  final List<String> _unitOptions = ['kg', 'g', 'liter', 'ml', 'piece', 'packet', 'box'];
  ...
  void initState() {
    ...
    if (widget.product != null) {
      ...
      _stockCountController.text = widget.product!['stock_count']?.toString() ?? '';
      _thresholdController.text = widget.product!['low_stock_threshold']?.toString() ?? '';
        ...
        if (response.statusCode == 200) {
          if (mounted) {
            setState(() {
               ...
               if (_selectedShopId != null && !_shops.any((s) => s['_id'] == _selectedShopId)) {
  ...
  Future<void> _submitProduct() async {
    ...
    if ((role == 'superadmin' || role == 'admin') && _selectedShopId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a branch explicitly.')));
      return;
    }
    ...
    try {
      ...
      final response = isUpdating
          ? await ApiService.put('${ApiConfig.baseUrl}/products/${widget.product!['_id']}', payload)
          : await ApiService.post('${ApiConfig.baseUrl}/products', payload);

      if (response.statusCode == 200 || response.statusCode == 201) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(isUpdating ? 'Product Updated' : 'Product Created Successfully')),
          );
  ...
  Widget build(BuildContext context) {
    ...
    final canSeeCostPrice = role == 'superadmin' || role == 'admin' || role == 'owner';
                ...
                children: [
                  if (isGlobalAdmin) ...[
                    DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(labelText: 'Store Assignment *', border: OutlineInputBorder()),
                    ...
                    children: [
                      if (canSeeCostPrice) ...[
                        Expanded(
                          child: TextFormField(
                            ...
                            validator: (val) => val!.isNotEmpty && double.tryParse(val) == null ? 'Invalid Number' : null,
                          ),
                        ),
                        const SizedBox(width: 16),
                      ],
                      Expanded(
                        child: TextFormField(
                          ...
                          validator: (val) => val == null || val.isEmpty || double.tryParse(val) == null ? 'Invalid Number' : null,
                        ),
                      ),
                    ],
                  ),
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          enabled: !isEditing, // Typically you adjust stock via Stock Log, not directly editing. But for creation it's fine.
                          keyboardType: TextInputType.number,
                          decoration: InputDecoration(
                            labelText: isEditing ? 'Current Stock (Use POS to adjust)' : 'Initial Stock Count *', 
                            border: const OutlineInputBorder(),
                            filled: isEditing,
                          ),
                          validator: (val) => !isEditing && (val == null || val.isEmpty || int.tryParse(val) == null) ? 'Required' : null,
                        ),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: TextFormField(
                          ...
                          validator: (val) => val!.isNotEmpty && int.tryParse(val) == null ? 'Invalid Number' : null,
                        ),
                      ),
                    ],
                  ),
                  ...
                  ElevatedButton(
                    ...
                    child: _isLoading 
                        ...
                        : Text(isEditing ? 'Save Changes' : 'Create Product', style: const TextStyle(color: Colors.white, fontSize: 18)),
                  )
                ],
              ),
            ),
